    ...
    // ============================================================
    // ROTA ADMINISTRATIVA - SINCRONIZAR / MIGRAR PLANOS COM A EFÍ PRODUÇÃO
    // ============================================================
    //
    ...
    // 2. NÃO confia cegamente em efi_plan_id já gravado.
    // 3. Para cada plano, monta o nome determinístico:
    //      Ache Obra - <nome_plano> - <UUID Supabase>
    // 4. Consulta GET /v1/plans na Efí usando as credenciais do ambiente
    //    atual (neste servidor: PRODUÇÃO) e procura esse nome exato.
    // 5. Se encontrar:
    //      - usa o plan_id retornado pela própria Efí Produção;
    //      - se o Supabase já tiver o mesmo ID, apenas confirma;
    //      - se o Supabase tiver ID antigo/diferente (ex.: Homologação),
    //        substitui pelo ID correto de Produção.
    // 6. Se não encontrar pelo nome, cria um novo plano mensal em Produção.
    // 7. Só depois grava efi_plan_id + status no Supabase.
    //
    // IMPORTANTE:
    // - Isso permite migrar com segurança IDs antigos de Homologação.
    // - Não apaga planos na Efí.
    // - Não cancela assinaturas.
    // - Não altera preços/cobranças existentes.
    // - Não cria duplicidade quando o plano determinístico já existe.
    // ============================================================
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.log(
            '>>> INICIANDO SINCRONIZAÇÃO / MIGRAÇÃO DOS PLANOS EFÍ PRODUÇÃO'
          );
          ...
          for (
            ...
            try {
              ...
              console.log(
                `>>> Verificando plano em PRODUÇÃO: ${nomePlano}`
              );

              console.log(
                `>>> ID atualmente salvo no Supabase: ${efiPlanIdAnterior || 'nenhum'}`
              );

              // A fonte de verdade é a Efí do ambiente atual.
              // Como localizarPlanoEfiPorNome usa o access token obtido
              // pelas credenciais de PRODUÇÃO, um plano de Homologação
              ...
              if (!planoEfi) {
                ...
                console.log(
                  `>>> Plano "${nomePlano}" não encontrado na Efí PRODUÇÃO. Criando...`
                );
              ...
              const planIdEfi =
                planoEfi?.plan_id !== undefined &&
                planoEfi?.plan_id !== null
                  ? String(
                      planoEfi.plan_id
                    ).trim()
                  : '';
              ...
              if (idMudou) {
                ...
                console.log(
                  `>>> MIGRAÇÃO DE ID: ${nomePlano} | antigo=${efiPlanIdAnterior} | produção=${planIdEfi}`
                );
              } else if (idJaCorreto) {
                jaSincronizados +=
                  1;
                ...
                console.log(
                  `>>> ID já válido em PRODUÇÃO: ${nomePlano} -> Efí ${planIdEfi}`
                );
              } else {
                console.log(
                  `>>> Novo vínculo de PRODUÇÃO: ${nomePlano} -> Efí ${planIdEfi}`
                );
              }
              ...
              console.log(
                `>>> Plano confirmado em PRODUÇÃO: ${nomePlano} -> Efí ${planIdEfi}`
              );
              ...
              resultados.push({
                ...
                status:
                  idMudou
                    ? 'id_migrado_para_producao'
                    : (
                        idJaCorreto
                          ? 'ja_sincronizado_producao'
                          : 'sincronizado_producao'
                      ),

                origem:
                  origem,

                efi_plan_id_anterior:
                  efiPlanIdAnterior ||
                  null,
              ...
              try {
                ...
                // Em caso de erro, NÃO apagamos nem substituímos o
                // efi_plan_id anterior. Apenas marcamos a sincronização
                // como erro para permitir diagnóstico e nova tentativa.
                await atualizarPlanoSupabase(
                  planoIdSupabase,
                  {
                    recorrencia_ativa:
          ...
          console.log(
            '>>> SINCRONIZAÇÃO / MIGRAÇÃO FINALIZADA'
          );

          console.log(
            `>>> Criados em Produção: ${criados}`
          );

          console.log(
            `>>> Reaproveitados em Produção: ${reaproveitados}`
          );

          console.log(
            `>>> IDs já corretos em Produção: ${jaSincronizados}`
          );

          console.log(
            `>>> IDs migrados/corrigidos: ${idsCorrigidos}`
          );
